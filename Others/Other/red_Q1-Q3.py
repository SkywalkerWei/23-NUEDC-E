...
while(True):
                ...
                for b in blob: # 区分开红、绿激光点
                    ...
                    if b[5]<(x_roi+w) and b[5] >x_roi and b[6]<(y_roi + h) and b[6]>y_roi:
                        ...
                        if (G-R)>30: # 绿色激光点
                            ...
                            print("绿色:",X_0,Y_0)
                        if len(ls_x) == send_len: #send_len是用来控制发送频率的
                            X_send = int(sum(ls_x)/send_len)
                            Y_send = int(sum(ls_y)/send_len)
                            send_data(X_send,Y_send)
                            print("send red_X:",X_send,'red_Y:',Y_send)
                            blue_led.toggle() #亮灯提示
                            ls_x=[]
                            ls_y=[]

            if len(blob) == 1 : 
                b = blob[0]
                # 这里要给roi重新设置初值，其他一模一样
                if b[5]<(x_roi+w) and b[5] >x_roi and b[6]<(y_roi + h) and b[6]>y_roi:
                    X_0 = b.cx()-W_C
                    Y_0 = -(b.cy()-H_C)
                    R,G,B = calculate_average_rgb(b.min_corners())
    ...
    if subscriber_mode ==3:
        ...
        if rec_detec_times < rec_detec_times_lim:
            ...
            for r in ACCCC:
                # 判断是否围成矩形
                is_rect = is_rectangle(r.corners()) # 判断余弦值
                area = calculate_area(r.corners()) #防止误判
                if is_rect and area>area_min:
                    ...
                    # send_data_8(X_3,Y_3,X_2,Y_2,X_1,Y_1,X_0,Y_0)
                ...
                if len(blob) == 1 : 
                    ...
                    if b[5]<(x_roi+w) and b[5] >x_roi and b[6]<(y_roi + h) and b[6]>y_roi:
                        ...
                        if len(ls_x) == send_len:
                            ...
                            red_x = X_send
                            red_y = Y_send
                            red_dected_flag = 1 # 这里加一个是否看到红点的标记
                            # send_data(X_send,Y_send)
                            # print("send red_X:",X_send,'red_Y:',Y_send)
                            blue_led.toggle()
                            ls_x=[]
        ...
        if red_dected_flag == 1: #识别到了红点
            # img.draw_arrow((red_x, red_y, X_3, Y_3), color = (0,0,255),size = 30, thickness = 2) # 画个箭头方便确认
            img.draw_cross(X_3, Y_3, size=30, color=(0,0,255))

            print("Red_X:",red_x,'Red_Y:',red_y) # 大写xy是顶点坐标
            ...
            send_data_8(red_relative_X,red_relative_Y, Point_2_X,Point_2_Y, Point_1_X,Point_1_Y, Point_0_X,Point_0_Y)
        else:
            pass

    # Q4.上述 A4 靶纸以任意旋转角度贴在屏幕任意位置 还没看
