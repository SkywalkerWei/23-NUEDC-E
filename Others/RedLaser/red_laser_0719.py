...
thresholds = [(0, 90, 127, 12, -128, 127)]
...
while(True): #不断拍照
    ...
    if True:
        #img = sensor.snapshot().lens_corr(1.8) #拍摄一张照片，lens_corr函数用于非鱼眼畸变矫正，默认设置参数为1.8
        #readflag=0
        for blob in img.find_blobs(thresholds,invert=True,pixels_threshold=2,area_threshold=2):
            #print(blob.solidity())
...
#            q[7]=q[7]//flag2
            ...
            if stage==3:
                ...
                if cnt==10:
                    ...
                    LED_tip()
                    if pos==8:
                        send_data_8(ppos[0],ppos[1],ppos[2],ppos[3],ppos[4],ppos[5],ppos[6],ppos[7])
                        LED(3).on()
                        stage=2
